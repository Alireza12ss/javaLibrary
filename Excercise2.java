            ...
            else
            {
                ...
                //reserve-seat
                    ...
                    if (let && libraries.get(edits[2]).getResday().containsKey(rooz) && libraries.get(edits[2]).getResday().get(rooz) == libraries.get(edits[2]).get_library_tedad_miz()) {
                        int tmp = 0;
                        for(seat i : libraries.get(edits[2]).getseats().get(rooz)){
                            // System.out.println(tmpp.getStart() + "  /////// " + i.getEnd());
                            if (tmpp.getStart().before(i.getEnd()) && tmpp.getEnd().after(i.getStart())) {
                                tmp++;
                            }
                        }
                        // System.out.println(tmp + "   :   " +libraries.get(edits[2]).get_library_tedad_miz() );
                        if (tmp >= libraries.get(edits[2]).get_library_tedad_miz()) {
                    ...
                    if(let){
                        if (libraries.get(edits[2]).getseats().containsKey(rooz)) {
                            libraries.get(edits[2]).getseats().get(rooz).add(tmpp);
                        }
                        else{
                            libraries.get(edits[2]).getseats().put(rooz ,new ArrayList<>());
                            libraries.get(edits[2]).getseats().get(rooz).add(tmpp);
                        }
...
class Library 
{
    ...
    private HashMap<Date, ArrayList<seat>> seats;
    ...
    public HashMap<Date, ArrayList<seat>> getseats() {
        return this.seats;
    }

    public void setseats(HashMap<Date, ArrayList<seat>> seats) {
        this.seats = seats;
    }
